...
def deliver_scheduled_email():
    ...
    logger.info(f"Received scheduled email send for '{nr_num}' from Cloud Tasks")

    if ce_cache.get(ce.id):
        logger.info(f"Skipping duplicate scheduled send for '{nr_num}'")
    ...
    if resp.status_code != HTTPStatus.OK:
        logger.error(f"Scheduled send failed for '{nr_num}'. Trying again...")
    ...
    logger.info(f"Scheduled email send completed for '{nr_num}'")
    return {}, HTTPStatus.OK
