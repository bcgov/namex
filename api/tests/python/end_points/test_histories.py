...
def test_no_match_because_stemmed_word(client, jwt, app):
    ...
    verify_same_or_similar(client, jwt,
                           query='GREAT NORTHERN OIL AND GAS LIMITED',
                           expected=None
                           )
...
@pytest.mark.parametrize("criteria, seed", [
    ...
    ('J\'s AND M\'s HOLDINGS', 'J AND M HOLDINGS',),
...
def test_multiple_similar_results_cases(client, jwt, app):
    ...
    url = SOLR_URL + '/solr/names/update?commit=true'
    headers = {'content-type': 'application/json'}
    data = [{"name":"J.J. TRUCKING", "id":"7", "name_state_type_cd":"R", "submit_count":"1", "nr_num":"NR 1"},
            {"name":"J.J.\'s TRUCKING INCORPORATED", "id":"8", "name_state_type_cd":"R", "submit_count":"1", "nr_num":"NR 2"},
            ...
            {"name":"J\'s and J\'s TRUCKING INC", "id":"21", "name_state_type_cd":"R", "submit_count":"1", "nr_num":"12"},
            {"name":"J-J\'S TRUCKING LTD", "id":"22", "name_state_type_cd":"R", "submit_count":"1", "nr_num":"NR 13"},
            {"name": "J 'n' J\'S TRUCKING LTD", "id": "23", "name_state_type_cd": "R", "submit_count": "1", "nr_num": "NR 14"},
            ]
...
def test_multiletter_similar_results_cases(client, jwt, app):
    ...
    url = SOLR_URL + '/solr/names/update?commit=true'
    headers = {'content-type': 'application/json'}
    data = [{"name":"JJJJESSSSICAAA\'SSS STALK EXCHANGE", "id":"7", "name_state_type_cd":"R", "submit_count":"1", "nr_num":"NR 1"},
    ...
    result = extract_list_of_values_for_key(search_histories(client, jwt, 'JESSICA\'S STALK EXCHANGE LTD')['names'], 'name')
    assert result
    assert len(result) == len(data)
