...
class InternalFilings(Resource):
    ...
    def patch(filing_id):
        ...
        if not jwt.validate_roles([COLIN_SVC_ROLE]):
            return jsonify({'message': 'You are not authorized to update the id'}), HTTPStatus.UNAUTHORIZED
...
class ColinLastUpdate(Resource):
    ...
    def get(colin_id=None):
        """Get the last id updated in legal."""
        if colin_id:
            ...
            if not colin_id:
                return {'message': f'No ids found'}, HTTPStatus.NOT_FOUND
