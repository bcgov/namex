...
def update_nr (nr, ora_cursor):
    ...
    _update_nro_names(ora_cursor, nr, eid)
...
def _update_request(oracle_cursor, nr, event_id):
...
def _update_nro_names(oracle_cursor, nr, event_id):
...
def _update_nro_address(oracle_cursor, nr, event_id):
    ...
    oracle_cursor.execute("""
    SELECT party_id, decline_notification_ind
    FROM request_party 
    WHERE request_id = :request_id
    AND end_event_id IS NULL
    ...
    row = oracle_cursor.fetchone()
    rp_id = int(row[0])
    decline_notification_ind = row[1]
    ...
    # get next address ID
    oracle_cursor.execute("""select address_seq.NEXTVAL@global_readonly from dual""")
    row = oracle_cursor.fetchone()
    ...
    # create new record for requst party instance
    oracle_cursor.execute("""
    ...
    """,
                          ...
                          decline_notification_ind=decline_notification_ind
                          )

def _update_nro_partner_name_system(oracle_cursor, nr, event_id):
