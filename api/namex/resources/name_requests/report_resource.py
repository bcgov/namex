...
class ReportResource(Resource):
        ...
        try:
            ...
            if not nr_model:
                return make_response(
                    jsonify(message='{nr_id} not found'.format(nr_id=nr_model.id)), HTTPStatus.NOT_FOUND
                )
    ...
    def _get_template_data(nr_model):
        ...
        nr_report_json = nr_modelentity_type_cd
        nr_report_json['service_url'] = current_app.config.get('NAME_REQUEST_URL')
        nr_report_json['entityTypeDescription'] = ReportResource._get_entity_type_description(
            nr_model['entity_type_cd']
        )
        ...
        instruction_group = ReportResource._get_instruction_group(
            nr_model['entity_type_cd'], nr_model['request_action_cd'], nr_model['corpNum']
        )
        ...
        nr_report_json['isPaper'] = not (
            ReportResource._is_colin(nr_model['entity_type_cd'])
            or ReportResource._is_modernized(nr_model['entity_type_cd'])
            or ReportResource._is_society(nr_model['entity_type_cd'])
        )
        nr_report_json['requestCodeDescription'] = ReportResource._get_request_action_cd_description(
            nr_report_json['request_action_cd']
        )
        nr_report_json['nrStateDescription'] = ReportResource._get_state_cd_description(nr_report_json['stateCd'])
        ...
        if nr_report_json['applicants']['countryTypeCd']:
            nr_report_json['applicants']['countryName'] = pycountry.countries.search_fuzzy(
                nr_report_json['applicants']['countryTypeCd']
            )[0].name
    ...
    def _is_magic_link(legal_type, request_action):
        magic_link_list = {'NEW': ['CR', 'UL', 'CC'], 'MVE': ['CR', 'UL', 'CC']}
    ...
    def _get_instruction_group(legal_type, request_action, corpNum):
        ...
        if ReportResource._is_magic_link(legal_type, request_action) and (
            (request_action == RequestAction.NEW.value and email_feature_flags.get('enable_won_emails'))
            or (request_action == RequestAction.MVE.value and email_feature_flags.get('enable_cont_in_emails'))
        ):
            return 'magic-link'
    ...
    def _get_next_action_text(entity_type_cd: str, url: str, instruction_group: str):
        ...
        next_action_text = {
            ...
            'SO': {'DEFAULT': f'To complete your filing, visit <a href="{url}">{url}</a> and login with your BCeID.'},
            'PA': {'DEFAULT': ReportResource.GENERIC_STEPS},
            'FI': {'DEFAULT': ReportResource.GENERIC_STEPS},
            'PAR': {'DEFAULT': ReportResource.GENERIC_STEPS},
            # XPRO and Foreign Types
            'XCR': {
                'NEW': f'Use this name request to complete your application by visiting <a href="{url}">{url}</a>',
                'CHG': f'Use this name request to complete your application by visiting <a href="{url}">{url}</a>',
            ...
            'XCP': {'DEFAULT': 'Extraprovincial Cooperative Association'},
            'XSO': {'DEFAULT': f'To complete your filing, visit <a href="{url}">{url}</a> and login with your BCeID.'},
            # Used for mapping back to legacy oracle codes, description not required
            'FIRM': {'DEFAULT': 'FIRM (Legacy Oracle)'},
        }
