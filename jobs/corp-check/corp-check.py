...
if __name__ == "__main__":
    ...
    # report out
    if processed < 0:
        print("corp sync: errored out: no rows process; completed in:{}".format(end_time - start_time)
              ,file=sys.stderr)
        exit(1)

    print("corp sync: finished - requests processed: {0} completed in:{1}".format(processed, end_time-start_time),
          file=sys.stderr)
    exit(0)
