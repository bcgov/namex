...
def nro_data_pump_update(nr, ora_cursor, expires_days=56):
    ...
    with tracer.start_active_span('nro_data_pump_update', child_of=parent_span) as scope:
        scope.span.set_tag(tags.DATABASE_TYPE, 'oracle')

        expiry_date = create_expiry_date(start=nr.lastUpdate, expires_in_days=expires_days, tz=timezone('US/Pacific'))

        scope.span.log_kv({'expiry_date': expiry_date})
        ...
        proc_parameters = [nr.nrNum,  # p_nr_number
        ...
        scope.span.set_tag(tags.DATABASE_STATEMENT, '{}({})'.format(proc_name, proc_parameters))

        # # Call the name_examination procedure to save complete decision data for a single NR
        ora_cursor.callproc(proc_name, proc_parameters)
