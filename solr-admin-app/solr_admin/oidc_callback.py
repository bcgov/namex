...
def callback():
    ...
    # If the user already has an access token, skip re-authentication
    if session.get("access_token"):
        current_app.logger.debug("User already has access token in session, redirecting to user-defined state URL.")
    ...
    current_app.logger.debug("Access token successfully saved to session, redirecting to %s", state)
    return redirect(state)
